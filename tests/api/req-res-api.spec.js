...
test("Should create a new user", async ({ request }) => {
    const response = await request.post("https://reqres.in/api/users", { data: userBody });
...
test("Should upload the user created", async ({ request }) => {
    const response = await request.put(`https://reqres.in/api/users/${userID}`, { data: { job: "Tester" } })
    const responseData = await response.json();
